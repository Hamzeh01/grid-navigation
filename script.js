...
function aStarSearch(grid, src, dest) {
  ...
  if (!isUnBlocked(grid, src[1], src[0]) || !isUnBlocked(grid, dest[1], dest[0])) {
      document.getElementById('output').innerText = "Source or the destination is blocked.";
      return;
  }

  if (isDestination(src[1], src[0], dest)) {
      document.getElementById('output').innerText = "We are already at the destination.";
      return;
  }

  let ROW = grid.length;
  let COL = grid[0].length;
  let closedList = new Array(ROW).fill(false).map(() => new Array(COL).fill(false));

  let cellDetails = new Array(ROW).fill(null).map(() => new Array(COL).fill(null).map(() => new cell()));
      ...
      for (let d of directions) {
          ...
          if (isValid(grid, newRow, newCol)) {
              if (isDestination(newRow, newCol, dest)) {
                  ...
                  if (cellDetails[newRow][newCol].f == 0 || cellDetails[newRow][newCol].f > fNew) {
  ...
  if (!foundDest) {
      document.getElementById('output').innerText = "Failed to find the destination cell.";
  }
}
      ...
      for (let d of directions) {
          ...
          if (isValid(grid, newRow, newCol)) {
              if (isDestination(newRow, newCol, dest)) {
                  ...
                  if (cellDetails[newRow][newCol].f == 0 || cellDetails[newRow][newCol].h > hNew) {
